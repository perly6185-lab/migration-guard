...
if current_state == 'RUNNING'
  and state ~= 'MAIN_COMMITTED'
  and state ~= 'FAILED' then
  return 'INVALID_TRANSITION'
end
if current_state == 'MAIN_COMMITTED'
  and state ~= 'COMPENSATION_RETRYING'
  and state ~= 'SUCCESS'
  and state ~= 'COMPENSATION_FAILED' then
  return 'INVALID_TRANSITION'
end
if current_state == 'COMPENSATION_RETRYING'
  and state ~= 'COMPENSATION_RETRYING'
  and state ~= 'SUCCESS'
  and state ~= 'COMPENSATION_FAILED' then
  return 'INVALID_TRANSITION'
...
if state ~= 'RUNNING'
  and state ~= 'FAILED'
  and requested ~= deleted + skipped then
