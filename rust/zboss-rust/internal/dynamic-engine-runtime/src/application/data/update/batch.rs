...
pub enum PlanError {
    ...
    HeaderRowsUnsupported { count: usize },
...
pub fn plan_batch(
    ...
    if header_row_count > 0 {
        return Err(PlanError::HeaderRowsUnsupported {
            count: header_row_count,
        });
    }
    if post_rows.len() > limit {
...
mod tests {
    ...
    fn plans_partial_success_and_mixed_insert_update() {
        ...
        let plan = plan_batch(&rows, 0, &[2], DEFAULT_ROW_LIMIT).unwrap();
    ...
    fn enforces_post_and_header_limits() {
        ...
        assert!(matches!(
            plan_batch(&[row.clone(), row.clone()], 0, &[], 1),
            Err(PlanError::RowLimitExceeded { .. })
        ));
        assert_eq!(
            plan_batch(&[row], 1, &[], 1),
            Err(PlanError::HeaderRowsUnsupported { count: 1 })
        );
    }
}
