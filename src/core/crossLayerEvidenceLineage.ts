...
export interface CrossLayerEvidenceLineageReport {
  ...
  assessmentScope: CrossLayerEvidenceLineageOptions;
  routeCount: number;
...
export async function assessCrossLayerEvidenceLineage(options: CrossLayerEvidenceLineageOptions): Promise<CrossLayerEvidenceLineageReport> {
  ...
  const sourceRevision = sourceIdentity.identity;
  ...
  const base = {
    version: 1 as const, createdAt, runId, root: analyzer.root, sourceRevision, sourceIdentity, assessmentScope: { ...options, root: analyzer.root }, routeCount: analyzer.routes.length, assessedCount: routes.length,
