...
test("Java call graph specializes literal-null branches without assuming variables are non-null", async () => {
  ...
  try {
    ...
    const textStrategyEdge = typedText.callGraph.edges.find((edge) => edge.to?.includes("TextFieldUpdateStrategy.apply"));
    assert.ok(textStrategyEdge && callGraphEdgeMatchesFieldTags(textStrategyEdge, { $fieldTag: ["FieldTagEnum.TEXT"] }));
...
test("advanced Java semantics cover inheritance, qualifiers, defaults, transactions, and language features", async () => {
  ...
  try {
    ...
    const files: Record<string, string[]> = {
      ...
      "EquivalentAttributeTransactionService.java": ["package demo;", "public class EquivalentAttributeTransactionService {", " @Transactional(rollbackFor = Exception.class)", " public Object save() { return null; }", " @Transactional(rollbackFor = Exception.class)", " public Object run() { return save(); }", "}"],
    ...
    assert.equal(createEndpointReplacementPlanFromJava(analyze("EquivalentAttributeTransactionService")).plan.findings.includes("RP-GRAPH-TRANSACTION-SELF-INVOCATION"), false);
