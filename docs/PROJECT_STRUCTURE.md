...
migration-guard/
  ...
  scripts/
    probes/
      ...
      md-api-contract-probe.mjs
      md-web-static-probe.mjs
    smoke/
...
- 为 `md-monorepo` adapter 生成 MD 整仓重构准备任务
- 校验 DAG 是否有缺失依赖或环
- 计算 ready tasks
- 更新任务状态
...
- 执行手动/AI 任务的记录路径
- 执行 `md-monorepo` adapter 的任务计划和 action plan 产物生成
...
- 在 GitHub live 前校验已确认的 plan hash
- 写入同步 evidence
